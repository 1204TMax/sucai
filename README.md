...
| 规则 | 原因 |
|------|------|
| **原生 HTML + 原生 JS** | 不得引入任何前端框架（React、Vue、Angular 等），不得使用 npm 构建流程 |
...
```
通用生成.html
  ├─→ 生成详情.html      生成完成后点击「查看详情」
  └─→ 工作流-裂变.html   点击工作流卡片（useWorkflow('image')）

生成详情.html
  ├─→ 全部图片.html      点击「查看全部」按钮（.card-act-more）
  ├─→ 全部图片.html      点击缩略图行末尾的「+N 查看全部」卡片（.view-all-card）
