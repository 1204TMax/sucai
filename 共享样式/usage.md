...
虽然基础间距定义在 `design-system.md` 中，但在实际搭建页面级框架排版时，请统一遵循以下最新规范，以确保与外层 iframe 和同级元素保持和谐的呼吸感：

- **页面外层边距 (`body padding`)**：统一使用 `8px`（不再使用 20px 或 4px）
- **页面内部大模块之间的缝隙 (`gap`)**：统一使用 `8px`（如顶部操作区与下方内容区之间、左侧分类树与右侧主网格之间的 gap）

> 使用 `8px` 的宏观间距配置能保证内容展示紧凑而不拥挤，是最适合当前系统卡片网格风格的默认值。

## 组件与排版精细化规范（基于最新迭代）

为了使整个系统（特别是包含大量卡片和树形结构的页面）保持紧凑、专业且高效的信息展示，请设计师与开发者统一遵循以下组件尺寸与排版规范（对齐“我的任务”等页面的紧凑风格）：

### 1. 基础控件尺寸（按钮、输入框等）
- **高度**：标准控件高度统一降至 **32px**（例如：`.btn`, `.form-control`, `.icon-btn`），不要使用 36px 这种偏大的尺寸。
- **内边距**：按钮的水平 padding 调整为 **12px**（即 `padding: 6px 12px`）。
- **字号**：基础操作按钮、输入框、下拉框内文字统一为 **13px**。

### 2. 文本字号与截断规则
- **标题类**：分类面板标题等通常使用 **14px**。
- **正文与辅助**：面包屑导航使用 **13px**；左侧分类树节点（`.tree-node`）、卡片标题、底部卡片标签（`.tag`）统一使用 **12px**。
- **卡片长文本截断**：素材标题等长文本统一使用 **2行截断**展示（基于 `-webkit-line-clamp: 2`），以便在有限空间内透出更多信息，而不要直接单行截断。

### 3. 标签与状态标记（Tags & Badges）
- **状态角标**：卡片右上角或状态标记，**弃用无意义的纯色圆点**，必须改用带底色和明确文字的标签（如 `.status-tag`）。例如：“未投放”（灰色）、“无消耗”（橙/黄色）、“有消耗”（主色/蓝色）。
- **标签排版防换行**：卡片底部的属性/内容标签容器必须**强制不换行**（`flex-wrap: nowrap`，`overflow: hidden`）。
- **标签溢出折叠（+X）**：当标签数量过多超出卡片宽度时，必须进行截断并动态显示一个灰底的 **`+X`** 标签（通过 JS 计算容器宽度实现）。同时，必须确保鼠标 hover 到标签区域时能通过原生的 `title` 或 Tooltip 查看所有完整的标签内容。

### 4. 空间与留白利用率（网格与控制栏）
- **控制栏（Toolbar）高度**：顶部控制栏、面包屑区域的上下 padding 应尽量收缩（如 **10px**），避免占用过多的纵向首屏空间。
- **网格间距（Gap）**：列表或卡片网格的间隙（gap）统一缩小至 **12px**，相应的容器内边距也保持 **12px**。
- **卡片最小宽度**：为提升屏幕空间的利用率，网格卡片的自适应最小宽度应适度收缩（如由 220px 降为 **180px**），以确保在一行内能展示更多素材。
...
**真实例子：**
- 项目弹窗看起来只是一个弹窗，但其实自带：搜索、三栏列表、多选逻辑、确认/取消、状态写回
